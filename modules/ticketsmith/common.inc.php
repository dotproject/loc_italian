...
/* $Id: common.inc.php,v 1.7 2003/02/12 19:07:33 kripper Exp $ */

/* program info */
$program = "PLA";
$version = "1";
$xmailer = "PLA (http://www.intera.it/pla/)";
...
function format_field ($value, $type, $ticket = NULL) {
    ...
    switch ($type) {
        case "user":
            if ($value) {
                $output = query2result("SELECT concat(user_first_name, ' ', user_last_name) as name FROM users WHERE user_id = '$value'");
            } else {
				$value = $appoggio;
                $output = "<a href=index.php?m=ticketsmith&a=view&ticket=$value><img src=images/icons/mailicon.gif border=0></a>";
            }
            break;
        case "status":
        ...
        case "assignment":
            $options[0] = "-";
            $query = "SELECT user_id as id, concat(user_first_name, ' ' , user_last_name) as name FROM users";
            $result = do_query($query);
        ...
        case "view":
            ...
            $output .= "<img src=images/icons/icn_view.gif border=0></a>";
            break;
        case "open_date":
